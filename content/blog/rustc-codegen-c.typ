...
For OSPP 2024, I applied for and was accepted to participate in the project #link("https://blog.rust-lang.org/2024/05/07/OSPP-2024.html")["C codegen backend for rustc"]. This is an exciting opportunity for me to engage with a community-driven open-source project for the first time.

The project involves developing a "backend" for rustc that translates Rust MIR to C code. This aims to enhance platform compatibility and potentially speed up compilation for rapid prototype development. The original concept was discussed on #link("https://rust-lang.zulipchat.com/#narrow/stream/122651-general/topic/rustc_codegen_c")[Zulip].
...
Rust offers more complex data structures compared to C. Its #link("https://doc.rust-lang.org/nomicon/repr-rust.html")[default memory model], `#[repr(Rust)]`, provides no stability guarantees, allowing Rust to optimize memory usage by rearranging fields for better efficiency.
...
Another issue is niche optimization. In Rust, `Option<Box<T>>` has the same size as `Box<T>` by using the null pointer "niche" to represent `None`. This optimization isn't straightforward to implement in C#footnote[Is niche optimization a MUST in Rust? As far as I know, there's no formal specification on this. However, according to #link("https://github.com/rust-lang/rust/pull/60300")[this PR], niche-optimized values are considered FFI-safe, and many crates rely on this feature. Ignoring it would likely be problematic.].
...
The domain of undefined behavior #link("https://www.youtube.com/watch?v=DG-VLezRkYQ")[differs between Rust and C]. There are instances where Rust code is well-defined but may lead to undefined behavior in C, and vice versa. The C codegen backend must ensure that the generated C code is free from undefined behavior.
...
However, Rust MIR explicitly marks out safe integer operations. These operations can be implemented by calling specific functions#footnote[The result type might be better defined as `struct { int err; int result; }`, which simulates `Option<i32>` in Rust.], such as:
