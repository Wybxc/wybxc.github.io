...
In this post, we'll explore the pretty printing algorithm used in #link("https://doc.rust-lang.org/nightly/nightly-rustc/rustc_ast_pretty/pp/index.html")[rustc], the Rust compiler, and utilized by the #link("https://crates.io/crates/prettyplease")[prettyplease] crate. This approach is based on Derek C. Oppen's paper, Pretty Printing@oppenPrettyprinting1980. While Oppen's pretty printer is less flexible than the widely used Wadler's pretty printer@wadlerPrettierPrinter2003, it offers advantages in terms of speed and memory efficiency.
...
2. The size of a `]` is 0#footnote[In the current implementation in rustc, the size of a `]` is set to 1, which is technically incorrect but doesn't affect the outcome since only the sign matters.].
3. The size of a breakable space or a `[` is determined by the total size from it (including its own space) to the next break point (or EOF)#footnote[According to the original paper, the size of a `]` should be the total size of its delimited block. The size of a space should include its own length plus the size of the next block. The implementation in rustc differs from it... I don't know why.]. If there is an entire delimited block between them, that block is treated as a single unit. Inner breakable spaces within the block do not affect the size calculation for the outer elements.
...
The implementation of the pretty printer is found in the #link("https://doc.rust-lang.org/nightly/nightly-rustc/rustc_ast_pretty/pp/index.html")[rustc_ast_pretty] crate. The interface looks like this:
...
More examples can be found in my #link("https://github.com/Wybxc/rustc_codegen_c/blob/master/crates/rustc_codegen_c_ast/src/pretty.rs")[rustc_codegen_c] project.
