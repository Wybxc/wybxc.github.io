...
I went on to learn that all computation can be expressed through functions (the lambda calculus), that recursion itself can be encoded as the mind-bending Y combinator:
...
Once you see the connections, the encoding's shape stops being a trick and starts being inevitable.

I want to trace those connections here. We'll start with the simplest data types, watch a pattern emerge, and build up the machinery: parametricity, algebraic data types, F-algebras, the Yoneda Lemma, until Church encoding looks less like a clever trick and more like something the math demands.
...
In System F, a function can take a type as an argument. You write type abstraction as $Λ X. med t$ and type application as $f med [X].$
...
Our System F starts as an empty universe.
In STLC, we would declare base types like $mono("Unit")$ and $mono("Bool")$ to get started. But in System F, it's interesting that we can build these types from scratch, using only functions and polymorphism. We can encode $mono("Unit")$ and $mono("Bool")$ as polymorphic functions, without any special syntax for data types.

Let me state a principle: a data type can be equivalently represented by the ways you consume it.
I can't yet explain the logic behind this claim, but let's set it aside and see how it works for $mono("Unit").$
#footnote[In the second half of this article, we'll see the category-theoretic foundation for this principle. The Yoneda Lemma will show that the set of all ways to consume a type fully determines the type itself.]
...
What inhabits this type? It is easy to think of one: the identity function.#footnote[
  Later we'll see that this is the only possible implementation, as a consequence of parametricity.
...
The Church encoding of a data type in System F is a polymorphic function whose arguments match the constructors.
It is a universal consumption interface for the type.
This works perfectly for $mono("Unit")$ and $mono("Bool"),$ whose constructors carry no data.
But what about a type like the $mono("Nat")$?
Its $mono("Succ")$ constructor has a recursive parameter.
We'll handle recursion soon.
But before we do, we need to understand the mechanism that makes the pattern work in the first place.

== Parametricity

What's striking is that for $∀X. med X → X,$ we can only find a single implementation, the identity function.
...
We'll start with a special case you can verify by hand.

*Claim*: $∀X. med X → X$ has only the identity function, i.e., any $f : ∀X. med X → X$ must satisfy $f med [A] med a = a$ for all $A$ and $a.$

*Proof*: Take two arbitrary types $A$ and $B,$ and pick any relation $R ⊆ A × B$ between them. The theorem requires us to lift $R$ through the type expression $τ(X) = X → X.$ The lifting works like this: two functions $h_A : A → A$ and $h_B : B → B$ are related by $τ(R)$ precisely when they map $R$-related inputs to $R$-related outputs, that is, $(h_A (x), h_B (y)) ∈ R$ whenever $(x, y) ∈ R.$ Intuitively, $h_A$ and $h_B$ preserve $R$ in lockstep.
...
Here comes the trick. Fix a type $A$ and a value $a : A.$ We want to show $f med[A] med a = a.$ Define a very specific relation that pairs $a$ with itself and nothing else:

$
  R = {(a, a)} ⊆ A × A
$

Take $B = A.$ Since $(a, a) ∈ R,$ parametricity forces:
...
Parametricity forced $f$ to be the identity function. $f$ knows nothing about the type it's applied to; the type signature alone left it no other choice.
...
Now let's look at the general shape $f : ∀X. med (F med X → X) → X$. Here $F$ is any type operator; think of it as describing the structure of a data type. The free theorem we get is this: for any F-algebra homomorphism#footnote[
  We'll explain what an F-algebra homomorphism is later.
] $h : (A, f_A) → (B, f_B),$
...
With parametricity in our toolkit, we now return to the main thread.
Recall where we left off: $mono("Unit")$ and $mono("Bool")$ revealed a pattern (Church encoding is a polymorphic function whose arguments match the constructors), but we had no way to handle recursive types like $mono("Nat").$ To extend the pattern, we need a systematic language for describing the structure of data types — something that makes constructors and their arguments explicit. This is where *algebraic data types* (ADT) come in.
...
Look at the expression $1 + X.$ That is precisely the body of the recursive type definition of $mono("Nat") = μ X. med 1 + X.$ In other words, the Church encoding is of the form:
...
In the next section, we'll see that $∀X. (F med X → X) → X$ is the type of universal consumers of *F-algebras*, and this is the perspective that will lead us, via parametricity and the Yoneda Lemma, to the deepest understanding of what Church encoding really is.

== F-Algebra

We've arrived at a turning point. The last section ended with a shape, $∀X. med (F med X → X) → X,$ that unifies the Church encodings of $mono("Unit"),$ $mono("Bool"),$ and $mono("Nat").$ But the shape still feels pulled from thin air. Why this particular arrangement of symbols? To answer that, we need to give names to the pieces. Category theory enters here.

A *category* is a collection of objects and arrows between them, with sensible rules for _composition_ and _identity_. For a programmer, the natural example is the category $bold("Set"):$ objects are types, arrows are functions, composition is function composition, and the identity arrow is `id`.
...
This is what the $F$ in $F med X = 1 + X$ has been doing: it builds a new type from an old one, *and* it lifts functions to act inside that structure.#footnote[For $mono("NatF"),$ given $f : X → Y,$ $mono("NatF") med f$ maps $mono("Left") med ()$ to $mono("Left") med ()$ and $mono("Right") med x$ to $mono("Right") med (f med x).$] This function-lifting is the machinery behind structural recursion; we'll return to it when we define folds.
...
The Church encoding type $∀X. med (F med X → X) → X$ now acquires a precise meaning: it is a *universal consumer of F-algebras*: given any F-algebra $⟨F, X, f⟩,$ it takes the evaluator $f : F med X → X$ and runs it on the data type's structure to produce a value of type $X.$ It makes the "consumption interface" idea rigorous.
...
These two maps point at each other: one wraps a $μ F$ value into a universal consumer, the other unwraps a universal consumer by running it on the initial algebra itself. We'll prove they are inverses, establishing $mono("ChurchF") ≅ μ F.$
...
One more consequence of initiality: $mono("fold") med mono("in") = mono("id").$ Since $mono("id")$ is a homomorphism from the initial algebra to itself, and initiality says there can only be one, the fold at the initial algebra's own structure map must be the identity. This equation unlocks the proof.

Stepping back: $mono("in")$ builds values from nothing. $mono("fold")$ takes those values and interprets them in any algebra you choose, uniquely. One end generates; the other end consumes. We call this a *universal producer*.
...
This is where parametricity pays off. Recall the free theorem: for any F-algebra homomorphism $h : (A, f_A) → (B, f_B),$

$
  h med (c med [A] med f_A) = c med [B] med f_B
$

Let $A = μ F,$ $f_A = mono("in"),$ $B = X,$ $f_B = f,$ and $h = mono("fold") med f.$ Since $mono("fold") med f$ is an algebra homomorphism, the free theorem applies directly:
...
Two independent ideas: the uniqueness of the fold from the initial algebra and the free theorem from parametricity, interlocked to deliver the isomorphism.

We've proved the isomorphism. But a question lingers: *why* this shape? Why $∀X. med (F med X → X) → X$? We arrived at it by following our noses through $mono("Unit"),$ $mono("Bool"),$ $mono("Nat"),$ and a bit of algebra. That's a fine derivation, but it doesn't explain why the shape *must* be what it is. For that, we turn to what is arguably the _most beautiful_ theorem in category theory.

== The Yoneda Lemma

Before stating the lemma, one more piece of vocabulary. The *Hom-functor* $sans("Hom")(A, −)$ embodies "the view from $A.$" For any object $X,$ $sans("Hom")(A, X)$ is the set of all arrows from $A$ to $X,$ which in $bold("Set")$ is just the function type $A → X.$ Given a function $g : X → Y,$ the Hom-functor maps it by post-composition: $sans("Hom")(A, g) = λ h. med g ∘ h.$ Formally: "everything you can do with an $A.$"
...
Beneath the formalism, the idea is simple. A natural transformation from $sans("Hom")(A, −)$ to $F$ is completely determined by a single choice: where you send $mono("id")_A.$ Pick any element of $F(A)$ as the image of $mono("id")_A,$ and naturality does the rest — it uniquely transports that choice to every other object. Conversely, given a natural transformation, applying it to $mono("id")_A$ picks out an element of $F(A).$ That's the whole correspondence.

The Hom-functor captures everything you can arrow out of $A$ — the "outward view" of $A.$ Yoneda says this outward view *completely determines* $A$ itself. Sound familiar? It's the categorical incarnation of our consumption-interface principle: a type is fully characterized by how you can consume it.

Let's make this concrete. Set $F$ to the identity functor $mono("Id")$ where $mono("Id")(X) = X$ and $cal(C)$ to the $bold("Set")$ category. The Yoneda lemma specializes to:

$
  sans("Nat")(sans("Hom")(A, −), med mono("Id")) ≅ A
$

Unpacking the left side: $sans("Hom")(A, X)$ is $A → X.$ A natural transformation $η : sans("Hom")(A, −) → mono("Id")$ is a family of functions $η_X : (A → X) → X,$ one for each $X.$
...
The universal shape of Church encoding, $∀X. med (F med X → X) → X,$ is the Yoneda representation of the initial algebra over the forgetful functor.

So the Church encoding is Yoneda in disguise. $∀X. med (F med X → X) → X$ is the left side of the Yoneda bijection for the initial algebra over the forgetful functor, and Yoneda guarantees it's isomorphic to $μ F.$

Parametricity and Yoneda are the same idea in two different languages. The free theorem $h med (c med [A] med f_A) = c med [B] med f_B$ is the Yoneda naturality square, instantiated in the category of F-algebras. The key proof step $mono("fold") med mono("in") = mono("id")$ is Yoneda's central move: determining the entire natural transformation from a single choice at $mono("id").$#footnote[The two proofs run in parallel: $mono("fold") med f med (c med [μ F] med mono("in")) = c med [X] med f$ is the naturality square applied to the homomorphism $mono("fold") med f;$ $mono("fold") med mono("in") = mono("id")$ corresponds to Yoneda's key step where $mono("id")$ determines the entire transformation. One path lives inside the type system; the other sits at a higher categorical level. They're two views of the same structure.] Parametricity is the type-theoretic shadow of categorical naturality.

== The Final Recap

Church encoding begins with a simple intuition: a data type can be replaced by the way you consume it. From $mono("Unit")$ and $mono("Bool"),$ this yields types like $∀X. med X → X$ and $∀X. med X → X → X.$ To handle recursive types, we introduce functors $F$ that describe a data type's one-layer structure, unifying all Church encodings under a single shape: $∀X. med (F med X → X) → X.$

The inner part $F med X → X$ is an *F-algebra*, and the Church encoding is the universal consumer of F-algebras: it takes any algebra and produces a value. Among all F-algebras, the initial algebra $μ F$ (the recursive data type itself) is the key one: its unique fold operation gives us $mono("fold") med mono("in") = mono("id").$ Meanwhile, parametricity forces polymorphic functions to behave as natural transformations, yielding the free theorem $h med (c med [A] med f_A) = c med [B] med f_B.$ These two facts — one from initiality, one from parametricity — interlock to prove that Church encoding is isomorphic to the original data type: $∀X. med (F med X → X) → X ≅ μ F.$

The Yoneda Lemma tells us why. Applied in the category of F-algebras, Yoneda states that the natural transformations from the Hom-functor out of the initial algebra to the forgetful functor are isomorphic to $μ F$ itself, and those natural transformations are precisely the inhabitants of $∀X. med (F med X → X) → X.$ The Church encoding is a direct consequence of one of category theory's most fundamental results. Parametricity and Yoneda are two views of the same structure: both rest on naturality, and one is the type-theoretic shadow of the other.

Returning to the Church numeral $λ s. med λ z. med s med (s med z):$ it is the Church representation of the initial algebra for the functor $mono("NatF").$ Church encoding is an elegant tool in programming, and its shape can be traced along a repeatable path from the consumption intuition all the way to the deep structure of category theory.
